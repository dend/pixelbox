...
A command-line tool for controlling Divoom pixel art devices (Ditoo Pro, Timebox Evo, Tivoo, Pixoo, etc.) over Bluetooth Low Energy from Linux.
...
Divoom devices are highlighted in the output. Grab the address (e.g. `AA:BB:CC:DD:EE:FF`).
...
The CLI only wires up a few commands right now, but the underlying protocol library (`src/commands/`) has full coverage of the Divoom binary protocol. This is the same protocol the official Android app uses over BLE. You can use it as a Rust library to build your own tooling.
...
| `commands::display` | Display modes (clock, temperature, color light, special effects, sound reactive, music visualization), screen rotation/mirror/on-off, light effects, work mode switching (BT/FM/SD/LineIn/USB) |
...
- BLE writes are chunked to 20 bytes (the Divoom firmware expects this).
...
The protocol was reverse-engineered from the Divoom Android app (v3.8.14). It should work with any Divoom device that uses BLE, including:

- Ditoo Pro / Ditoo Pro 5
- Ditoo Plus / Ditoo Mic
- Timebox Evo / Timebox Evo Pro
- Tivoo / Tivoo Max / Tivoo LCD
- Pixoo (BLE models)
- Timoo, MiniToo, Zooe

Older devices that use classic Bluetooth SPP instead of BLE (Timebox Mini, original Tivoo) are not supported -- they need an RFCOMM serial connection, not GATT.
